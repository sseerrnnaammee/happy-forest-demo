...
$_ready(function () {
	...
	async function setSlots() {
		...
		// console.log(label,gameProcess)
		// if(gameProcess) {
		// 	analyseStatement(label[gameProcess]);
		// }else {
		// 	resetGame()
		// }
	...
	function startGame(){
		...
		if(gameProcess && gameProcess.endDateStatus){
			...
			if(nextDay > now){
				...
				// 更新endDateStatus字段为false
				const playerId = Storage.get('playerId');
				gameProcess.endDateStatus = false
				gameProcess.endDate = ''
				const data = JSON.stringify(gameProcess)
				updateGameProcess(playerId, data).then((res) => {
		...
		setTimeout(() => {
			$_("section").hide();
			$_("[data-menu='animation'] [data-ui='load-progress']").value(0);
			$_("#game").show();
			if (gameProcess) {
				loadFromSlot(gameProcess)
			} else {
				resetGame()
				analyseStatement(label[0]);
			}
		}, 2100)
	}
	function newSave(name) {
		...
		if (playing) {
			...
			const playerId = Storage.get('playerId');
			const data = JSON.stringify({ [engine.SaveLabel + id]: saveData })
			updateGameProcess(playerId, data).then((res) => {
				setSlots()
			})
		}
	...
	function showMainMenu() {
		...
		$("[data-menu='main'] .account").text(Storage.get("playerName"));
	}
	...
	/**
	 * Description: 判断用户是否已经登录, 若无跳转登录页, 已登陆则跳转到主菜单页面
	 * ==========================
	 **/
	...
	/**
	 * Description: 当日场景已经结束后跳转到等待界面
	 * ==========================
	 **/
	function jumpWaiting(Scene){
		// $_("[data-menu='waiting']").style("background", "url(img/scenes/" + scenes[Scene] + ") center / cover no-repeat");
		$_("[data-menu='waiting']").show();
	}
	/**
	 * ==========================
	 * 过渡动画
	...
	**/
	function runAnimate() {
		let count = 1
		$_("#game").hide();
		$_("[data-menu='animation']").show();
		let timer = setInterval(() => {
			if (count > 10) {
				clearInterval(timer)
			} else {
				$_("[data-menu='animation'] [data-ui='load-progress']").value(10 * count);
				count++
			}
		}, 200)

		$_("[data-menu='loading']").fadeOut(2000, function () {
			$_("[data-menu='loading']").hide();
		});
	}
	/**
